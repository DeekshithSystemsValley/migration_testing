...
class Sheet(models.Model):
    ...
    week_start = fields.Many2one('date.range',string="Week Start")
    week_end = fields.Many2one('date.range',string="Week End")

    @api.onchange('week_start', 'week_end')
    def onchange_week(self):
        self.date_start = self.week_start.date_start
        self.date_end = self.week_end.date_end
    ...
    def _compute_line_ids(self):
        ...
        for sheet in self:
            ...
            for key in sorted(matrix,
                              key=lambda key: self._get_matrix_sortby(str(key))):
                vals_list.append(sheet._get_default_sheet_line(matrix, key))
                if sheet.state in ['new', 'draft']:
                    print("----------matrix key",matrix[key])
                    sheet.clean_planning(matrix[key])
            sheet.line_ids = SheetLine.create(vals_list)

    def clean_planning(self, planning):
        repeated = planning.filtered(lambda t: t.name == empty_name)
        print("--------------repeated entry ",repeated)
        if len(repeated) > 1 and self.id:
            return repeated.merge_planning()
        return planning
    @api.multi
    def _get_data_matrix(self):
        ...
        # print("------------------planning isd",self.planning_ids)
        ...
        for date in self._get_dates():
            ...
            # week_end = self.env['date.range'].search([('date_end','=',self.date_end)])
            if week_id:
                # print("--------date from get dates",date)
                # print("------week id name",week_id.name)
    ...
    def _get_dates(self):
        ...
        while start != end:
            ...
            # print("---------_get_dates start date---------",start)
            dates.append(start)
        return dates
    ...
    def _get_default_sheet_line(self, matrix, key):
        self.ensure_one()
        week_date = self._get_date_name(key.date)
        print("------_get_default_sheet_line",week_date)
        # for t in matrix:
        #     print("------------unit amount",t)
        week_id = self.env['date.range'].search([('name','=',week_date),('type_id.calender_week','=',True)])
        values = {
    ...
    def _matrix_key_attributes(self):
        """ Hook for extensions """
        return ['date', 'project_id', 'employee_id']
    ...
    def _compute_planning_ids(self):
        ...
        for sheet in self:
            ...
            planning = AccountAnalyticLines.search(domain)
            sheet.link_planning_to_sheet(planning)
            sheet.planning_ids = planning
    ...
    @api.multi
    def action_planning_draft(self):
    ...
    @api.multi
    def action_planning_confirm(self):
    ...
    @api.multi
    def action_planning_refuse(self):
        if self.filtered(lambda sheet: sheet.state != 'confirm'):
    ...
    def button_add_line(self):
        for rec in self:
            if rec.state in ['new', 'draft']:
                print("-----triggering line function")
                rec.add_line()
                print("-----triggering line function triggere")
                rec.reset_add_line()
    ...
    def _prepare_empty_analytic_line(self):
        return {
            ...
            'date': self.date_start,
            'project_id': self.add_line_project_id.id,
            'task_id': self.add_line_emp_id.id,
    ...
    def link_planning_to_sheet(self, planning):
        self.ensure_one()
        if self.id and self.state in ['new', 'draft']:
            for aal in planning.filtered(lambda a: not a.planning_sheet_id):
                aal.write({'planning_sheet_id': self.id})
    ...
    def _is_add_line(self, row):
        ...
        return self.add_line_project_id == row.project_id \
            and self.add_line_emp_id == row.task_id
    ...
    def _prepare_new_line(self, line):
        """ Hook for extensions """
        return {
            'planning_sheet_id': line.planning_sheet_id.id,
            'date': line.date,
        ...
        if r == WEEKLY:
            if company.timesheet_week_start:
                delta = relativedelta(
                    ...
                    days=6)
                print("------delta if company week start",delta)
            else:
                delta = relativedelta(days=date.weekday())
                print("------delta if company doesnt have start date",delta)
            return date - delta
        elif r == MONTHLY:
            return date + relativedelta(day=1)
            print("------delta monthly",delta)
        return date
    ...
    def _track_subtype(self, init_values):
        ...
        if 'state' in init_values and self.state == 'confirm':
            return 'magnus_planning.mt_timesheet_confirmed'
        elif 'state' in init_values and self.state == 'done':
            return 'magnus_planning.mt_timesheet_approved'
...
class SheetNewAnalyticLine(models.TransientModel):
    ...
    def _update_analytic_lines(self):
        sheet = self.planning_sheet_id
        planning = sheet.planning_ids.filtered(
            lambda aal: self._is_similar_analytic_line(aal)
        )
        new_ts = planning.filtered(lambda t: t.name == empty_name)
        amount = sum(t.unit_amount for t in planning)
        diff_amount = self.unit_amount - amount
        if len(new_ts) > 1:
            new_ts = new_ts.merge_planning()
